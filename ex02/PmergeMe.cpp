...
t_iv block_vector::get_B_block(const unsigned int i) const
{
    ...
    const t_iv::const_iterator block_start =
        (i == max_B_i() && max_B_i() > max_A_i()) ? _vector.begin() + 2 * (i - 1) * block_size
...
t_id block_deque::get_B_block(const unsigned int i) const
{
    ...
    const t_id::const_iterator block_start =
        (i == max_B_i() && max_B_i() > max_A_i()) ? _deque.begin() + 2 * (i - 1) * block_size
...
std::vector<int> validate_input(int c, char **v)
{
    std::vector<int> ret;
...
std::deque<int> validate_input_deque(int c, char **v)
{
    std::deque<int> ret;
